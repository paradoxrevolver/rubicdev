...
*/
/*
  This function represents a single instance of a penrose triangle navigation.

  Given a <canvas> HTML element, it modifies that and its immediate parent container
  to become a spinning penrose triangle that interacts with the mouse and provides
  a list of navigation buttons.
*/
var penrose = function (canvas) {
    if (!(canvas instanceof HTMLElement))
        throw "penrose instance did not receive an HTMLElement";
    ...
    _.Init = function () {
        ...
        _.MIN_LENGTH =
            penroseParent.clientWidth < penroseParent.clientHeight
                ? penroseParent.clientWidth
                : penroseParent.clientHeight;
        ...
        _.buttonArray = [
            {
                ...
                FollowUrl: () => {
                    ...
                    else window.location.href = "/delta";
                },
            },
            {
                text: "ME",
    ...
    _.Update = function () {
        ...
        _.cursor.centerDist = Math.sqrt(
            Math.pow(_.cursor.x - _.center.x, 2) +
                Math.pow(_.cursor.y - _.center.y, 2)
        );
        ...
        _.buttonStyle.font =
            _.buttonStyle.fontSize + "px " + _.BUTTON_STYLE.FONT_FAMILY;
        ...
        _.buttonArray.forEach((button, i, buttonArray) => {
            ...
            button.y =
                _.center.y +
                ((i - (buttonArray.length - 1) / 2.0) *
                    _.buttonStyle.verticalSpread -
                    _.buttonStyle.fontSize / _.BUTTON_STYLE.TEXT_BOTTOM_SHIFT);
            ...
            button.bgBox = {
                up: _.buttonStyle.fontSize / 2.0 + _.buttonStyle.padding.up,
                right:
                    (_.buttonStyle.fontSize / 2.0) *
                        button.text.length *
                        _.BUTTON_STYLE.TEXT_CHAR_WIDTH +
            ...
            button.loading = _.Lerp(
                button.loading,
                button.goal,
                _.BUTTON_STYLE.INTERPOLATE_SPEED
            );
        });

        _.penroseProgress.goal = _.Clamp(
            0.0,
            ((_.PENROSE_PROGRESS.FULL_POINT - _.PENROSE_PROGRESS.GONE_POINT) /
                (_.PENROSE_PROGRESS.CEILING_DISTANCE -
                    _.PENROSE_PROGRESS.FLOOR_DISTANCE)) *
        ...
        if (!_.cursor.isPresent) {
            _.penroseProgress.goal = 1;
        }

        // lerp penrose loading towards goal
        _.penroseProgress.current = _.Lerp(
            _.penroseProgress.current,
        ...
        _.penroseProgress.currentFifth = Math.min(
            _.penroseProgress.current * 5,
            1
        );
        ...
        if (_.DEBUG.PRINT_CURSOR_POSITION) {
            console.log(
                "CURSOR POSITION: (" + _.cursor.x + ", " + _.cursor.y + ")"
            );
    ...
    _.DrawHighlightedText = function () {
        ...
        _.buttonArray.forEach((button) => {
            ...
            p.lineTo(
                button.x + button.bgBox.right,
                button.y + button.bgBox.down
            );
            p.lineTo(
                button.x + button.bgBox.left,
                button.y + button.bgBox.down
            );
            p.closePath();
            ...
            if (p.isPointInPath(_.cursor.x, _.cursor.y)) {
                ...
                if (_.cursor.isClicking) _.TravelToButtonLink(button);
            } else {
                button.goal = 0;
            }
            ...
            p.beginPath();
            p.moveTo(
                button.x + button.bgBox.left * button.loading,
                button.y + button.bgBox.up
            );
            p.lineTo(
                button.x + button.bgBox.right * button.loading,
                button.y + button.bgBox.up
            );
            p.lineTo(
                button.x + button.bgBox.right * button.loading,
    ...
    _.DrawPenrose = function () {
        ...
        if (_.DEBUG.PRINT_PENROSE_PROGRESS)
            console.log(
                _.penroseProgress.current.toFixed(4) +
                    " -> " +
                    _.penroseProgress.goal.toFixed(4)
            );
        ...
        if (_.penroseProgress.current < 1) _.UpdatePenroseL();
    ...
    _.UpdatePenroseL = function () {
        let leftHalf = _.PercentOfPath(
            _.penroseL.slice(0, 4),
            _.penroseProgress.current
        );
        let rightHalf = _.PercentOfPath(
            _.penroseL.slice(4, 7).reverse(),
            _.penroseProgress.current,
            1.75
        ).reverse();
        ...
        for (let i = 1; i < distances.length; i++) {
            ...
            // otherwise, we need to cut some fraction of this next distance
                ...
                newPath.push({
                    x:
                        points[i - 1].x +
                        (points[i].x - points[i - 1].x) * piecePercentage,
                    y:
                        points[i - 1].y +
                        (points[i].y - points[i - 1].y) * piecePercentage,
                });
    ...
    _.TravelToButtonLink = function (button) {
        button.FollowUrl();
    };

    _.SetCursorClickState = function (state) {
        _.cursor.isClicking = state;
    };
    _.ResetCursorClickState = function () {
        _.cursor.isClicking = false;
    };

    _.SetCursorStyle = function (style) {
        document.body.style.cursor = style;
        _.cursor.isStyled = true;
    };
    _.ResetCursorStyle = function () {
        if (_.cursor.isStyled) document.body.style.cursor = null;
        _.cursor.isStyled = false;
    };
    ...
    document.body.addEventListener("mouseleave", () => {
        _.cursor.isPresent = false;
    });
    document.body.addEventListener("mousemove", (event) => {
        _.UpdateMousePosition(event);
    });
    document.body.addEventListener("click", () => {
        _.SetCursorClickState(true);
    });
...
// inner scope so currentScript is scoped to this script
{
    ...
    $(() => {
        let canvas = $(currentScript).siblings("canvas.penrose-canvas")[0];
        new penrose(canvas);
    });
}
